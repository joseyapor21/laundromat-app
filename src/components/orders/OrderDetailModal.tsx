...
export default function OrderDetailModal({ order, onClose, onUpdate, currentUser }: OrderDetailModalProps) {
  ...
  // Print order receipts - with options (uses printerService for ESC/POS formatting with QR codes)
  ...
  // Print bag labels (uses printerService for ESC/POS formatting with QR codes)
  const handlePrintLabel = async (bagIndex?: number) => {
    ...
    try {
      if (bagIndex !== undefined) {
        ...
        toast.success('Bag label printed!');
      } else {
        await printerService.printBagLabels(currentOrder);
        toast.success(`${currentOrder.bags?.length || 0} bag label(s) printed!`);
      }
    } catch (error) {
